...
Canlıdaki hataları otonom olarak çözen bir hat kurun. Bunu manuel VPS cron'u ile yapabileceğiniz gibi, Claude Code'un 2026'da gelen yerleşik **Scheduled Tasks** (zamanlanmış görevler) özelliğiyle herhangi bir harici cron ihtiyacı olmadan da yapabilirsiniz.
...
[Sentry yakalar]
       ↓ webhook
[Linear/GitHub Issues'da Bug kartı açılır]
       ↓ zamanlanmış görev (Scheduled Task veya cron)
[Triage Ajanı çalışır]
       ↓
[İzole worktree + branch açılır (--worktree flag)]
       ↓
[Claude Code sorunu çözer, Batch API ile ucuz maliyet]
       ↓
[Testleri ve lint'i kendisi çalıştırır]
       ↓
[Başarılıysa PR açar, başarısızsa hata notu bırakır]
       ↓
[Sabah insan gelir ve sadece review yapar]
```

#### Yerleşik Scheduled Tasks (Harici Cron Gerektirmeyen Yöntem)
Claude Code v2.1+ sürümünde bulut tabanlı zamanlanmış görevler doğrudan desteklenir. Herhangi bir VPS veya cron kurulumu yapmadan Claude'a "bunu her 10 dakikada bir kontrol et" diyebilirsiniz.

Konuşma içinde:
```
/loop 10m
"Linear'da yeni 'Todo' durumunda Bug kartlarını kontrol et.
 Yeni bir bug bulursan, yeni bir worktree'de arka planda
 (claude --bg) düzeltmeye başla.
 Sonucu bana bildir."
```

Veya yapılandırılmış görev (`.claude/tasks/nightly-audit.json`):
```json
{
  "name": "bug-triage",
  "schedule": "*/10 * * * *",
  "prompt": "Linear'daki işlenmemiş Bug'ları kontrol et, otonom olarak düzelt.",
  "model": "sonnet",
  "maxBudgetUsd": 2,
  "maxTurns": 10,
  "isolation": "worktree"
...
Avantajı: Bilgisayarınız kapalı olsa bile görev bulut üzerinde çalışır. Sonuçları döndüğünüzde görürsünüz.

#### Manuel Cron Yöntemi (Kendi VPS'inizde)
...
"""
    ...
    # Tmux'ta ajanı başlat
    tmux_session = f"bug_{safe_name}"
    cmd = f"cd {worktree} && claude --print 'TASK.md oku ve hatayi duzelt. Bittiginde beni bir not ile bilgilendir.' > agent.log 2>&1"
...
Her işi en iyi yapan modele verin. Bir modeli her şey için kullanmak **pahalı ve yavaş**tır. Fiyatlar Ağustos 2026 güncel haliyle:

| Görev Tipi | Tavsiye Model | Input/Output $/MTok | Neden |
|:---|:---|:---|:---|
| Zor mimari tasarım, yeni konsept, çok karmaşık debug | Claude Fable 5 | $10/$50 | En yüksek muhakeme kapasitesi |
| Mimari kararlar, ADR yazma, zor algoritma | Claude Opus 5 | $5/$25 | Güçlü muhakeme, yaratıcı çözüm |
| Ana kod geliştirme, günlük işler | Claude Sonnet 5 | $2/$10 (promo) veya $3/$15 | En iyi fiyat/performans |
| Boilerplate/CRUD/toplu işler | Claude Haiku 4.5 | $1/$5 | Çok hızlı ve ucuz, tekrarlayan iş |
| Zero-context adversarial review | GPT-5 (farklı aile) | ~$1.25/$10 | Farklı kör noktalar, robotik titizlik |
| Çok ucuz, yüksek hacimli boilerplate | DeepSeek-V4 Flash | $0.14/$0.28 | 10x+ ucuz, basit kod için yeterli |
| Büyük refactor | Claude Sonnet 5 | $3/$15 | Kod tabanı bağlamını iyi tutar |
| Test yazma (toplu), dokümantasyon | Haiku 4.5 / DeepSeek Batch | $0.50/$2.50 (batch) | %50 indirimli batch API ile daha ucuz |
| Zor hata ayıklama | Claude Opus 5 + GPT-5 çapraz |  | İki farklı perspektifden doğrulama |

**İpuçları:**
- Acil olmayan toplu işler (toplu test, toplu refactor) için **Batch API** kullanın; hem input hem output %50 ucuz.
- Tekrarlayan büyük sistem prompt'ları ve dosyalar **otomatik cache'lenir**, cache hit'te input %90 indirimli (örn: Sonnet'te $0.20/MTok).
- Basit işler için `/effort low` kullanmak hem hızlandırır hem de gereksiz token tüketimini önler.

**Claude Code'ta model değiştirme:**
```bash
# Oturum içinde anında değiştir
/model opus    # Claude Opus 5
/model sonnet  # Claude Sonnet 5 (varsayılan)
/model haiku   # Claude Haiku 4.5
/model fable   # Claude Fable 5 (en güçlü)

# Kalıcı varsayılan ayar
claude config set model sonnet

# Komut satırı bayrağı ile
claude --model haiku -p "tüm dosyalardaki TODO'ları listele"
```

### 6.5 Eval ve Kalite Ölçüm Sistemi
